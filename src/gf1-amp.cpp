/* GF1 Amp Command - Version 3.0 for Debian Linux
...
int main(int argc, char **argv)
{
    ...
    if (argc < 2) {  // If the program was called without arguments
        ...
        if (amplitude < GF1Device::AMPLITUDE_MIN || amplitude > GF1Device::AMPLITUDE_MAX) {  // If the obtained amplitude value (in Vpp) after conversion is lesser than "AMPLITUDE_MIN" [0] or greater than "AMPLITUDE_MAX" [5]
            std::cerr << "Error: Amplitude should be between 0 and 5Vpp.\n";
            errlvl = EXIT_USERERR;
        } else {
            GF1Device device;
            ...
            if (err == GF1Device::SUCCESS) {  // Device was successfully opened
                ...
                if (errcnt > 0) {  // In case of error
                    ...
                    std::cout << "Amplitude set to " << std::fixed << std::setprecision(2) << GF1Device::expectedAmplitude(amplitude) << "Vpp." << std::endl;  // Print the set amplitude
                }
                device.close();
